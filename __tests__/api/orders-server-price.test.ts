...
function validatePrices(
  ...
  const serverTotal = serverSubtotal + clientShipping
...
const sampleOffers: OfferRow[] = [
  { id: 'offer-1', product_id: 'prod-a', variant_id: null, price: 100.0 },
  { id: 'offer-2', product_id: 'prod-b', variant_id: null, price: 250.5 },
  { id: 'offer-3', product_id: 'prod-c', variant_id: 'var-1', price: 75.0 },
  // offer-4: prod-a'nın daha pahalı teklifı — en ucuzu seçilmeli
  { id: 'offer-4', product_id: 'prod-a', variant_id: null, price: 120.0 },
]
...
describe('Sunucu tarafı fiyat doğrulama', () => {
  it('fiyatlar eşleşince doğrulama geçmeli', () => {
    ...
    const result = validatePrices(items, 200.0, 20.0, 220.0, sampleOffers)
    expect(result.ok).toBe(true)
    if (result.ok) {
      expect(result.serverSubtotal).toBe(200.0)
      expect(result.serverTotal).toBe(220.0)
  ...
  it('tolerans dahilindeki küçük fiyat farklarını (≤ 0.01 ₺) kabul etmeli', () => {
    ...
    const result = validatePrices(items, 100.005, 0, 100.005, sampleOffers)
    expect(result.ok).toBe(true)
  })
  ...
  it('istemci fiyatı sunucu fiyatından farklıysa 409 dönmeli', () => {
    ...
    const result = validatePrices(items, 50.0, 0, 50.0, sampleOffers)
  ...
  it('istemci total sunucu total\'ından farklıysa 409 dönmeli', () => {
    ...
    const result = validatePrices(items, 100.0, 20.0, 100.0, sampleOffers)
    expect(result.ok).toBe(false)
    if (!result.ok) {
      expect(result.status).toBe(409)
  ...
  it('offers listesinde olmayan ürün 400 döndürmeli', () => {
    ...
    const result = validatePrices(items, 99.0, 0, 99.0, sampleOffers)
    expect(result.ok).toBe(false)
    if (!result.ok) {
  ...
  it('en ucuz teklifi seçmeli (çoklu teklif senaryosu)', () => {
    ...
    const result = validatePrices(items, 100.0, 0, 100.0, offersReordered)
  ...
  it('variant_id ile ayrı ayrı fiyat takip etmeli', () => {
    ...
    const result = validatePrices(items, 225.0, 0, 225.0, sampleOffers)
    expect(result.ok).toBe(true)
    if (result.ok) {
  ...
  it('birden fazla ürün doğru hesaplama yapmalı', () => {
    ...
    expect(result.ok).toBe(true)
    if (result.ok) {
      expect(result.serverSubtotal).toBeCloseTo(450.5, 2)
      expect(result.serverTotal).toBeCloseTo(expectedTotal, 2)
      expect(result.serverPricedItems).toHaveLength(2)
    }
  })
})
...
describe('order_items snapshot alanları', () => {
  it('serverPricedItems array doğru formatta olmalı', () => {
    ...
    const result = validatePrices(items, 200.0, 0, 200.0, sampleOffers)
    expect(result.ok).toBe(true)
    if (result.ok) {
  ...
  it('variant_id null olarak normalize edilmeli (undefined değil)', () => {
    ...
    const result = validatePrices(items, 100.0, 0, 100.0, sampleOffers)
    expect(result.ok).toBe(true)
    if (result.ok) {
