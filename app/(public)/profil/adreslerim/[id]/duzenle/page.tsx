...
import {
  ArrowLeft, Loader2, Check, MapPin, AlertCircle, RefreshCw
} from 'lucide-react'
...
export default function AdresDuzenlePage() {
  ...
  const fetchAddress = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(json.error || 'Adres bulunamadı')
      }

      const address: Address = json.data
      reset({
    ...
    try {
      ...
      if (!res.ok) {
        if (json.details) {
          ...
          toast.error(json.error || 'Adres güncellenirken bir hata oluştu')
        }
        return
      }
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 bg-background-card rounded-lg animate-pulse" />
          <div>
            <div className="h-8 w-48 bg-background-card rounded animate-pulse mb-2" />
  ...
  if (!user) {
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-text-primary">Adres Düzenle</h1>
        </div>
        <div className="bg-background-card rounded-xl border border-border p-8 text-center">
          ...
          <p className="text-text-secondary mb-4">
            Adres düzenlemek için giriş yapmanız gerekiyor.
          </p>
          <Link href="/giris">
      ...
      <div className="space-y-6">
        ...
        <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-6 text-center">
          ...
          <div className="flex gap-3 justify-center">
            <Button onClick={fetchAddress} variant="outline">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-text-primary">Adres Düzenle</h1>
          <p className="text-text-secondary">Adres bilgilerinizi güncelleyin</p>
        </div>
      </div>
